...
#endif
VOID CmpSetIrqDataWrk(VOID);
VOID CmpSetIrqDataWrkCurrent(VOID);
...
--*/
{
    ...
    RtlInitAnsiString( &AnsiString, HIROSHIMA_TAG);
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        DPRINT("IRQINIT: CreateKey of %wZ failed - Status == %lx\n",
                 &NameString, status);
    }
	...
	if (!NT_SUCCESS(status)) {
        DPRINT("IRQINIT: SetValueKey of %wZ failed - Status == %lx\n",
                 &NameString, status);
    }
    ...
    )
{
    ...
    UNICODE_STRING  NameString;
    UNICODE_STRING  ValueString;
    HANDLE          key1, key2, key3, key4, key5, key6;
    UCHAR           WorkString[128];
    WCHAR           ValueBuffer[128];
    OBJECT_ATTRIBUTES   ObjectAttributes;
    NTSTATUS            status;
	...
	RtlInitUnicodeString(
        &NameString,
        L"2BD63D28D7BCD0E251195AEB519243C13142EBC3"
        );
    ...
    )
{
	...
	RtlInitUnicodeString(
        &NameString,
        L"FEE449EE0E3965A5246F000E87FDE2A065FD89D"
        );
