...
ULONG CmNtSpBuildNumber = HIROSHIMA_REVISION;
const GUID NtBuildGUID = {0xffffffff, 0xffff, 0xffff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
...

{
    ...
    if (Number == 0) {
        ...
        while ((NextEntry != &LoaderBlock->LoadOrderListHead) && (ImageCount < 2)) {
          ...
          DbgPrint("-----------------------------------------------------\n"
                   "Shorthorn(%s-r%s) (Build %s, CSD: %d)\n"
                   "Command Line: %s\n"
                   "ARC Paths: %s %s %s %s\n"
                   "-----------------------------------------------------\n",
                   HIROSHIMA_TAG, CmNtSpBuildNumber,
...

{
    ...
    OldTime.QuadPart = Int32x32To64(10, -(10 * 1000 * 1000));
    Status = ZwWaitForSingleObject(
                ProcessInformation->Thread,//ProcessInformation->Process,
                FALSE,
                &OldTime
                );
...
BOOLEAN InbvEnableLDDM()
{
  struct _RTL_QUERY_REGISTRY_TABLE QueryTable; // [sp+4h] [bp-40h]@3
  int v2; // [sp+20h] [bp-24h]@3
  int v3; // [sp+24h] [bp-20h]@3
  int v4; // [sp+28h] [bp-1Ch]@3
  int v5; // [sp+2Ch] [bp-18h]@3
  int v6; // [sp+30h] [bp-14h]@3
  int v7; // [sp+34h] [bp-10h]@3
  int v8; // [sp+38h] [bp-Ch]@3
  int v9; // [sp+3Ch] [bp-8h]@1
  int v10; // [sp+40h] [bp-4h]@1

  v10 = 1;
  v9 = 1;
  return (QueryTable.EntryContext = &v10,
          QueryTable.DefaultType = 4,
          QueryTable.DefaultLength = 4,
          QueryTable.QueryRoutine = 0,
          QueryTable.Flags = 32,
          QueryTable.Name = L"UseXPModel",
          v2 = 0,
          v3 = 0,
          v4 = 0,
